...
void IndexController::onFrameEnd() {
	...
	if (this->_needToInsertBuilding) {
		...
		try {
			...
			auto level = building.at("level").get<unsigned int>();
			std::string name = building.at("name").get<std::string>();
			bool resources = building.find("resourcesRequired") != building.end();
			auto iron = resources ? building.at("resourcesRequired").get<nlohmann::json>().at("iron").get<unsigned int>() : 0;
			auto crystal = resources ? building.at("resourcesRequired").get<nlohmann::json>().at("crystal").get<unsigned int>() : 0;
			auto iridium = resources ? building.at("resourcesRequired").get<nlohmann::json>().at("iridium").get<unsigned int>() : 0;
			auto energy = resources ? building.at("resourcesRequired").get<nlohmann::json>().at("energy").get<unsigned int>() : 0;
