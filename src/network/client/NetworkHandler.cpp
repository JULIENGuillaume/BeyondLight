...
bl::network::client::NetworkHandler::NetworkHandler(std::string const &ip, unsigned short port) : m_networkClient(std::make_shared<BeyondLightClient>(this)) {
...
bl::network::client::NetworkHandler::~NetworkHandler() {
	this->m_networkClient->disconnect();
	this->m_networkThread->join();
}

std::string bl::network::client::NetworkHandler::getLine() {
...
void bl::network::client::NetworkHandler::send(std::string const &cmd) {
...
std::future<std::string> bl::network::client::NetworkHandler::asyncGetLine() {
	std::future<std::string> future = std::async(&NetworkHandler::getLine, this);
	return future;
}

void bl::network::client::NetworkHandler::asyncSend(std::string const &cmd) {
	this->send(cmd);
}

void bl::network::client::NetworkHandler::addWatcher(bl::network::client::EWatcherType type, std::string const &name, std::function<void(std::string const &)> fct) {
	this->m_watchers.emplace(type, std::make_pair(name, fct));
}

void bl::network::client::NetworkHandler::notifyWatchers(bl::network::client::EWatcherType type) {
